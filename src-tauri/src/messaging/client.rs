...
pub async fn send_to_device<T: serde::Serialize>(
    ...
    println!("[PEACOCK-DEBUG] Attempting TCP connect to {}", target_addr);
    let mut stream = TcpStream::connect(target_addr)
        .await
        .map_err(|e| {
            println!("[PEACOCK-DEBUG] TCP connect to {} FAILED: {}", target_addr, e);
            PeacockError::Network(format!("Cannot connect to {}: {}", target_addr, e))
        })?;
    println!("[PEACOCK-DEBUG] TCP connected to {} OK", target_addr);
