...
## 网络架构
- **UDP 52000**: 设备发现 + 消息/信令 (组播 224.0.1.100 + 定向广播 + 有限广播)
- **动态 TCP 端口**: 文件传输（接收方开端口，发送方连接）
- **发现规则**:
  1. 我广播 → 对方 AnnounceResponse 回应 → 我加对方到设备列表
  2. 我收到广播 → 发现自己在对方 restricted_peers 里 → 我加对方
  3. 收到广播不直接添加设备（只回 AnnounceResponse）
- **受限状态**: 基于 `last_broadcast_at` 时间戳，超过 30 秒未收到广播标为 restricted（黄点）
...
## 已完成功能
- 设备发现：收敛式发现机制（广播回应 + restricted_peers 自发现）
- 广播受限设备标识（黄点 / 绿点）
- 即时消息 (UDP)
- 文件/文件夹传输 (TCP, 拖拽、进度条、断点续传、文件夹 manifest)
- 片段功能 (CRUD、搜索、分享到设备、快速复制芯片、自动保存)
- 设置面板 (设备名、下载目录、自动接收)
- UI: Bandcamp Style + 无边框窗口 + 统一标题栏 + 暗色主题
- 跨平台: Windows、Linux、iOS (Swift 原生)、Android (Kotlin/Compose)

## 待办清单
- 截图功能（Phase 1: xcap 截屏 + 框选 UI，Phase 2: 窗口吸附，Phase 3: 标注）
- 消息/文件接收系统通知
